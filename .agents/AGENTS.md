...
**What it cannot do:**
- Read `/sandbox/finance`, `/sandbox/content`, or `/sandbox/build`
...
Verify all five sandbox policies contain this mount. Run Phase A tests first.
...
Currently defines 24 message type schemas.
...
These eight rules apply in every deployment — solo or squad:
...
/sandbox/analytics/reports/weekly-intelligence.json
```
This file must be configured as a read-only mount in **all five** claw
...
**Development phase:** All inference routes to cloud (Nemotron 120B).
...
Primary template for development and testing. All five claws, one operator,
...
│   │       ├── build_scheduler.py
│   │       └── build_claw.py
│   ├── roles/                           5 role blueprints
│   ├── policies/                        5 sandbox policies

...
#   --nous-oauth          Use Nous Portal OAuth (enables managed tool gateways)
...
ENVIRONMENT VARIABLES (for non-interactive mode):
    ...
    NEMOCLAW_NOUS_OAUTH          1 to use Nous Portal OAuth
...
EXAMPLES:
    ...
    ./install-hermes.sh --model-router --nous-oauth --slack-channels "#alerts,#general"
...
check_model_router_python() {
    ...
    if [[ -n "$python_path" ]]; then
        if [[ -x "$python_path" ]]; then
            local version=$("$python_path" -c "import sys; print(f'{sys.version_info.major}.{sys.version_info.minor}')")
...
prompt_nous_oauth() {
    ...
    echo ""
    log_info "Nous Portal OAuth vs API Key mode:"
    echo "  - Nous OAuth: Enables managed tool gateways (web search, browser automation, image generation, audio, managed code execution)"
    echo "  - API Key: Standard NVIDIA inference only"
    echo ""
    read -rp "Use Nous Portal OAuth for managed tool gateways? [y/N]: " -n 1
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        NOUS_OAUTH=true
    fi
...
main() {
    ...
    if [[ "$NON_INTERACTIVE" == "true" ]]; then
        ...
        NOUS_OAUTH="${NEMOCLAW_NOUS_OAUTH:-false}"
    ...
    log_info "  Nous OAuth: $NOUS_OAUTH"

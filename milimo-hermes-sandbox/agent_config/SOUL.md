...
output format) are enforced by the Finance Claw context when the skill is active.
Do not inline spend instructions here. Trust the tool return values; do not
explore the filesystem to understand spend mechanics.
...
1. TUI: Open http://localhost:9090/warroom.html in a browser. Click Approve or Veto buttons -- POST routes dispatch to the correct handler via the live action handler registry.

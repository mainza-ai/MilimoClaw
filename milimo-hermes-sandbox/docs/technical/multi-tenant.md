...
> [!NOTE]
> Database isolation in MilimoClaw is designed and implemented via **logical row-level security using a `Tenant ID` column** rather than physical PostgreSQL schema boundaries. Requests are filtered and routed with matching `X-Tenant-ID` headers to ensure application-level query scoping. Physical postgres schema partitioning is out-of-scope for v1.

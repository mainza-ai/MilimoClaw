...
function restoreSnapshotToHost(snapshotDir, logger) {
    ...
    if (typeof manifest.homeDir !== "string" || !isWithinRoot(manifest.homeDir, trustedRoot)) {
        logger.error(`Snapshot manifest homeDir is outside the trusted host root. ` +
    ...
    if (envStateDir) {
        ...
        if (normalizeHostPath(manifest.stateDir) !== normalizeHostPath(resolvedEnvStateDir)) {
            logger.error(`Snapshot manifest stateDir does not match OPENCLAW_STATE_DIR. ` +
        ...
        logger.error(`Snapshot manifest stateDir is outside the trusted host root. ` +
    ...
    if (manifest.hasExternalConfig) {
        ...
        if (typeof manifest.configPath !== "string" || !manifest.configPath.trim()) {
            logger.error(`Snapshot manifest has hasExternalConfig=true but configPath is missing or empty. Refusing to restore.`);
            return false;
        }
        ...
        if (envConfigPath) {
            ...
            if (normalizeHostPath(manifest.configPath) !== normalizeHostPath(resolvedEnvConfigPath)) {
                logger.error(`Snapshot manifest configPath does not match OPENCLAW_CONFIG_PATH. ` +
            ...
            logger.error(`Snapshot manifest configPath is outside the trusted host root. ` +

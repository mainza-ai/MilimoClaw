...
class HermesDelegateAdapter(DelegationAdapter):
    ...
    CLAW_CONTEXTS: dict[str, str] = {
        ...
        "finance": (
            ...
            "0. MANDATORY FIRST ACTION — SPEND FLOWS: When the operator request "
            "involves payment, purchase, subscription, SaaS provisioning, or API "
            "credit purchase, your FIRST action MUST be `milimo_spend`. Do NOT "
            "run `which`, `ls`, `find`, `cat`, `grep`, read source files, write "
            "Python scripts, or any other probing command first. milimo_spend "
            "performs its own auth check, payment-method discovery, and "
            "validation. If it returns an error, surface the error and stop.\n"
            "1. FORBIDDEN — DO NOT CREATE MOCKS OR WRAPPER SCRIPTS FOR EXTERNAL "
            "BINARIES: If `link-cli` or any other external tool is missing, "
            "unauthenticated, or returning an error, you must report the error "
            "to the operator. You must NOT write a Python mock, shell wrapper, "
            "or fake binary, and must NOT prepend a new directory to PATH to "
            "shadow a real installation. Doing so silently breaks the real tool "
            "for every subsequent call in this and future sessions.\n"
            "2. FORBIDDEN — DO NOT IMPORT milimo_core FINANCE CLASSES DIRECTLY: "
            ...
            "parameter validation, auth prechecks, and _finance_context "
            "injection. Use only the registered tools (milimo_spend, "
            "milimo_warroom).\n"
            "3. TOOL-FIRST: Call registered tools (milimo_spend, milimo_warroom) "
            "directly. Do NOT explore the filesystem, walk directories, or open "
            "source files to understand how tools work.\n"
            "4. APPROVAL URL (verbatim surfacing): If milimo_spend or "
            ...
            "5. NO SELF-NAVIGATION: You MUST NOT attempt to open, visit, navigate, "
            ...
            "6. STOP AND WAIT: After surfacing the approval_url, STOP. "
            ...
            "7. AUTH INITIATION (via helper only): If _check_link_cli_auth returns "
            ...
            "  3. FORBIDDEN: do NOT call link-cli auth login directly, do NOT retry\n"
            "     payment-methods list, do NOT call milimo_spend until auth status confirms.\n"
            "\n"
            "LOOP PREVENTION:\n"
            "  - Each call to `link-cli auth login` directly creates a NEW device "
            "code and invalidates previous approval URLs. This destroys any "
            "approval the operator just completed and forces them to start over. "
            "If you catch yourself about to run `auth login` directly, STOP. "
            "Run `_run_link_cli_auth_login()` helper instead, or run "
            "`auth status` if rechecking after approval.\n"
            "\n"

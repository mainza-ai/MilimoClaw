...
export class GatewayClient {
    ...
    private pendingAcks: Map<string, { resolve: () => void; reject: (err: Error) => void; timeout: NodeJS.Timeout }> = new Map();
    ...
    private sendFileMessage(message: GatewayMessage): void {
        const queueDir = join(process.env.HOME ?? "/tmp", ".milimo", "mesh", "pending", this.squadId, message.recipient_role);

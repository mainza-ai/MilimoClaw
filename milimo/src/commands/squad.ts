...
export function checkFinalsModeAutoResume(logger: PluginLogger): void {
  ...
  if (today >= finalsMode.resumeDate) {
    logger.info("");
    logger.info(
      `⏰ Finals Mode resume date (${finalsMode.resumeDate}) reached. Auto-resuming operations.`,
    );
...
export function cliSquadStatus(opts: SquadStatusOptions): Promise<void> {
  const { logger } = opts;
...
export async function cliSquadFinalsMode(opts: FinalsModeOptions): Promise<void> {
  ...
  const finalsModeState: FinalsModeState = {
    ...
    previousPolicies: (
      (data["policy_changes"] as Array<{ claw: string; previous: string }>) ?? []
    ).reduce(
      (acc, change) => {
  ...
  const policyChanges = data["policy_changes"] as
    | Array<{
        claw: string;
        previous: string;
        new: string;

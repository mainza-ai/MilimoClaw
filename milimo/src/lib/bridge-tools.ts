...
import { callPythonBridgeSafe, type BridgeResponse, type BridgeCommandOptions } from "./python-bridge.js";
...
export interface MeshFlowStateResult {
	nodes: Record<string, { status: string; address: string; last_heartbeat: string | null; pending_messages: number }>;
...
export interface OpsProjectsResult {
	projects: Array<{ name?: string; id: string; status: string; client_id?: string; projects?: Array<{ id: string; status: string; client_id?: string }> }>;
	sandbox_exists: boolean;
}

export interface ContentDraftsResult {
	drafts: Array<{ id: string; status: string; platform?: string; content_type?: string; client_id?: string }>;
	sandbox_exists: boolean;
}

export interface BuildPrsResult {
	prs: Array<{ number: number; title: string; author: string; createdAt: string; updatedAt: string; labels: string[]; url: string }>;
	gh_available: boolean;
}

export interface AnalyticsReportResult {
	report: { filename: string; content: Record<string, unknown> } | { filename: string; error: string } | null;
...
export interface WeeklyReportResult {
	generated_at: string;
	week_start: string;
	claws: Record<string, { role: string; tool_count: number; health: unknown; pending_messages: number }>;
	report_path: string;
}

export interface DeadlineCheckResult {
	checked_at: string;
	deadlines: Array<{ claw: string; type: string; deadline: string; days_remaining: number; status: string }>;
...
export interface DependencyAuditResult {
	status: string;
	audit_path: string;
	audits: Array<{ type: string; outdated_count?: number; vulnerabilities?: unknown; error?: string }>;
	audited_at: string;
}

export interface DiscoverToolsResult {
	claws: Record<string, { tools: Array<{ name: string; version: string }>; count: number; last_evolution: string | null }>;
	total_tools: number;
...
export class BridgeTools {
	 ...
	 */
	async clawStatus(args: { role: string; squad_id?: string }): Promise<BridgeResponse<ClawStatusResult>> {
	 ...
	 */
	async sendToClaw(args: { role: string; type: "assistant_query" | "assistant_task"; payload: Record<string, unknown>; squad_id?: string }): Promise<BridgeResponse<SendToClawResult>> {
	 ...
	 */
	async generateSprintPlan(args?: { instructions?: string; backlog_source?: string }): Promise<BridgeResponse<SprintPlanResult>> {
	 ...
	 */
	async runOpportunityScoring(args?: { criteria?: string[]; scope?: string }): Promise<BridgeResponse<OpportunityScoringResult>> {
	 ...
	 */
	async generateWeeklyReport(args?: { squad_id?: string; week_start?: string }): Promise<BridgeResponse<WeeklyReportResult>> {

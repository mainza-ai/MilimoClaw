 ...
 */
function detectActiveChannels(): ChannelStatus[] {
  const channels: ChannelStatus[] = [];
  const checks: Array<{ name: NotificationChannel; envKey: string }> = [
...
function formatDigestForChannel(brief: DigestBrief): string {
  const lines: string[] = [];
  ...
  if (brief.type === "morning") {
    ...
    if (brief.pending_actions && brief.pending_actions.length > 0) {
      ...
      for (const action of brief.pending_actions.slice(0, 5)) {
        const emoji: Record<string, string> = { HOLD: "🔴", REVIEW: "🟡", AUTO: "🟢" };
        lines.push(
          `  ${emoji[action.priority] ?? "⚪"} [${action.id.slice(0, 8)}] ${action.claw}: ${action.type}`,
        );
      }
    }
  } else {
...
function formatAlert(level: AlertLevel, text: string): string {
 ...
 * messaging platform.
 */
function deliverToOutbox(message: string, logger: PluginLogger): boolean {
  try {
    const sandboxOutbox = "/sandbox/.openclaw/milimo/mesh/outbox/notifications";
    const home = process.env.HOME || process.env.USERPROFILE || "/tmp";
    const homeOutbox = join(home, ".openclaw/milimo", "mesh", "outbox", "notifications");
    const outbox = existsSync(sandboxOutbox) ? sandboxOutbox : homeOutbox;
    ...
    writeFileSync(join(outbox, filename), JSON.stringify(payload, null, 2));
    logger.debug(`[milimo] Notification queued: ${filename}`);
    return true;
  } catch (err) {
    logger.warn(
      `[milimo] Failed to queue notification: ${err instanceof Error ? err.message : String(err)}`,
    );
    return false;
  }
}
...
export class ChannelNotifier {
  ...
  private channelCache: ChannelStatus[] | null = null;
  ...
  public getActiveChannels(): ChannelStatus[] {
    const now = Date.now();
  ...
  public activeChannelNames(): string[] {
   ...
   */
  public sendDigestBrief(brief: DigestBrief): boolean {
   ...
   */
  public sendHoldAlert(holdMessage: {
    message_id: string;
    sender_role: string;
    message_type: string;
    amount?: number;
   ...
   */
  public sendAlert(level: AlertLevel, text: string): boolean {
   ...
   */
  public sendCostGuardWarning(remaining: number, limit: number): boolean {
    if (!this.config.costGuardAlertsEnabled) {
      return false;
    }

    const pct = Math.round((remaining / limit) * 100);
    const level: AlertLevel = remaining <= 0 ? "critical" : "warning";
    const text =
      remaining <= 0
 ...
 */
export function loadNotificationConfig(): Partial<NotificationConfig> {
  try {
    const sandboxConfig = "/sandbox/.openclaw/milimo/config.json";
    const home = process.env.HOME || process.env.USERPROFILE || "/tmp";
    const homeConfig = join(home, ".openclaw/milimo", "config.json");
    const configPath = existsSync(sandboxConfig) ? sandboxConfig : homeConfig;

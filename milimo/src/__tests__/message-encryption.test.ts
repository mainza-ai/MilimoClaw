...
describe("MessageEncryption", () => {
    ...
    describe("key derivation", () => {
        it("produces consistent keys for same roles", () => {
            const key1 = (encryption as unknown as { deriveKey: (s: string, r: string) => Buffer }).deriveKey("content", "ops");
            const key2 = (encryption as unknown as { deriveKey: (s: string, r: string) => Buffer }).deriveKey("content", "ops");
        ...
        it("produces different keys for different role pairs", () => {
            const key1 = (encryption as unknown as { deriveKey: (s: string, r: string) => Buffer }).deriveKey("content", "ops");
            const key2 = (encryption as unknown as { deriveKey: (s: string, r: string) => Buffer }).deriveKey("ops", "content");

            expect(key1.equals(key2)).toBe(false);
        });

        it("produces 32-byte keys", () => {
            const key = (encryption as unknown as { deriveKey: (s: string, r: string) => Buffer }).deriveKey("content", "ops");

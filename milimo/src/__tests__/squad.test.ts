...
/**
 * Jest tests for squad finals-mode commands
 */

jest.mock("../lib/python-bridge", () => ({
    callPythonBridgeSafe: jest.fn((command: string, args: Record<string, unknown>) => {
        if (command === "activate_deep_work") {
            return {
                ...
                data: {
                    ...
                    policy_changes: [
                        { claw: "content", previous: "normal", new: "pause_drafts", blocked_actions: ["publish", "send"] },
                        { claw: "ops", previous: "normal", new: "maintenance", blocked_actions: ["new_outreach", "follow_up"] },
                        { claw: "analytics", previous: "normal", new: "passive", blocked_actions: ["experiment", "test"] },
                        { claw: "finance", previous: "normal", new: "invoices_only", blocked_actions: ["new_invoice", "new_client"] },
                        { claw: "build", previous: "normal", new: "issues_only", blocked_actions: ["open_pr", "merge"] },
                    ],
                },
            };
        }
...
describe("bridge integration", () => {
    ...
    describe("activate_deep_work", () => {
        it("calls bridge with correct parameters", () => {
            const { callPythonBridgeSafe } = require("../lib/python-bridge");
            callPythonBridgeSafe("activate_deep_work", { resume_date: "2026-04-01" }, { blueprintDir: "/tmp" });
        ...
        it("returns policy changes per claw", () => {
            const { callPythonBridgeSafe } = require("../lib/python-bridge");
            const result = callPythonBridgeSafe("activate_deep_work", { resume_date: "2026-04-01" }, { blueprintDir: "/tmp" });

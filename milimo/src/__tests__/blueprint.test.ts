...
/**
 * Jest tests for Blueprint commands
 */
...
jest.mock("../commands/init", () => ({
	...
	saveMilimoState: jest.fn(),
}));

const mockedFs = jest.requireMock("node:fs");
const mockedSpawnSync = jest.requireMock("node:child_process").spawnSync as jest.Mock;
...
describe("Blueprint Commands", () => {
	...
	describe("cliBlueprintPublish", () => {
		...
		it("uses default name from squad and role", async () => {
			...
			expect(logger.info).toHaveBeenCalledWith(expect.stringContaining("my-squad-content-blueprint"));
		});
	});

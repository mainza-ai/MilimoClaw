...
class MetricsCollector:
    ...
    def record_message_processed(
        self, message_type: str, processing_time_ms: float
    ) -> None:
    ...
    def record_inference_call(
        self, data_type: str, tokens: int, latency_ms: float
    ) -> None:
    ...
    def record_sla_compliance(
        self, message_type: str, sla_ms: float, actual_ms: float
    ) -> None:
                    ...
                    for line in f:
                        ...
                        try:
                            ...
                            if metric_type == "message_processed":
                                persisted_counters["messages_processed"] = (
                                    persisted_counters.get("messages_processed", 0) + 1
                                )
                                msg_type = tags.get("message_type", "unknown")
                                key = f"messages.{msg_type}"
                                persisted_counters[key] = (
                                    persisted_counters.get(key, 0) + 1
                                )
                                ...
                                persisted_counters["errors"] = (
                                    persisted_counters.get("errors", 0) + 1
                                )
                                ...
                                persisted_counters["inference_calls"] = (
                                    persisted_counters.get("inference_calls", 0) + 1
                                )
                                data_type = tags.get("data_type", "unknown")
                                persisted_counters[f"inference.{data_type}"] = (
                                    persisted_counters.get(f"inference.{data_type}", 0)
                                    + 1
                                )
                                tokens = int(tags.get("tokens", "0"))
                                persisted_counters["inference_tokens"] = (
                                    persisted_counters.get("inference_tokens", 0)
                                    + tokens
                                )
                                ...
                                counter = (
                                    "sla_compliant" if compliant else "sla_violation"
                                )
                                persisted_counters[counter] = (
                                    persisted_counters.get(counter, 0) + 1
                                )
                                msg_type = tags.get("message_type", "unknown")
                                persisted_counters[f"sla.{msg_type}.{counter}"] = (
                                    persisted_counters.get(
                                        f"sla.{msg_type}.{counter}", 0
                                    )
                                    + 1
                                )
        ...
        for key in set(list(persisted_timings.keys()) + list(self._timings.keys())):
            ...
            if combined:
                merged_timings[key] = {
                    ...
                    "p95_ms": round(sorted(combined)[int(len(combined) * 0.95)], 2)
                    if len(combined) >= 20
                    else round(max(combined), 2),
                }

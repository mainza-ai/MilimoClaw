...
class BlueprintMerger:
    ...
    @staticmethod
    def merge(
        base: BlueprintSnapshot, incoming: BlueprintSnapshot
    ) -> BlueprintSnapshot:
        """
        Merge incoming blueprint into the base blueprint.
        Returns a new merged blueprint snapshot (without integrity digest).
        """
        merged_tools = BlueprintMerger._merge_tools(
            base.tools_inventory, incoming.tools_inventory
        )
        merged_policy = BlueprintMerger._merge_policies(base.policy, incoming.policy)
        merged_priors = BlueprintMerger._merge_priors(
            base.learned_priors, incoming.learned_priors
        )
        ...
        merged_meta.niche_tags = list(
            set(base.meta.niche_tags + incoming.meta.niche_tags)
        )
        ...
        logger.info(
            f"Merged blueprints. Base v{base.meta.version} + Incoming v{incoming.meta.version}"
        )
        return snapshot

    @staticmethod
    def _merge_tools(
        base_tools: Dict[str, Any], incoming_tools: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    @staticmethod
    def _merge_policies(
        base_policy: Dict[str, Any], incoming_policy: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for k, v in incoming_policy.items():
            ...
            if isinstance(base_val, bool) and isinstance(v, bool):
                ...
                if "threshold" in k.lower() or "limit" in k.lower():
                    merged[k] = (
                        min(base_val, v) if "limit" in k.lower() else max(base_val, v)
                    )
                else:
    ...
    @staticmethod
    def _merge_priors(
        base_priors: Dict[str, Any], incoming_priors: Dict[str, Any]
    ) -> Dict[str, Any]:
